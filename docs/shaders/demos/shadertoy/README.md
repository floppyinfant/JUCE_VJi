...
ShaderToy Help (?) in Editor: GLSL Hilfe
...
uniform sampler2D   iChannel0;              // 2D texture
uniform samplerCube iChannel1;              // Cube texture
...
// Soundcloud (512x2, 1ch int8): takes in a URL
// --------------------------------
...
// --------------------------------
// Image: fragment shader main() / entry point
void mainImage(out vec4 fragColor, in vec2 fragCoord);
// Common:
vec4 someFunction(vec4 a, float b);
// Buffer A..D: can be loaded with iChannel0..3
void mainImage(out vec4 fragColor, in vec2 fragCoord);
// Cube A: can be loaded with iChannel0..3
void mainCubemap(out vec4 fragColor, in vec2 fragCoord, in vec3 rayOri, in vec3 rayDir);
// Sound: returns a vec2 containing the left and right (stereo) sound channel wave data
...
Diese Hilfe deckt nur jene Bereiche von GLSL ES ab die relevant für Shadertoy sind. Eine vollständige Spezifikation ist hier zu finden:
[GLSL ES Spezifikation](https://registry.khronos.org/OpenGL/specs/es/3.0/GLSL_ES_Specification_3.00.pdf)
