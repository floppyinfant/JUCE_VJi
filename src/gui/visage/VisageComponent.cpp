...
// examples/BringYourOwnWindow
int bringYourOwnWindow() {
...
public:
    ...
    visage::Brush computeBrush(double render_time) const {
        visage::Gradient rainbow(0xffff6666, 0xffffff66, 0xff66ff66, 0xff66ffff, 0xff6666ff, 0xffff66ff, 0xffff6666);
        ...
        visage::Gradient boost = visage::Gradient::fromSampleFunction(256, [boost_phase](float t) {
            return visage::Color(1.0f, 1.0f, 1.0f, 1.0f, 1.0f + std::max(0.0f, 0.4f - 3.0f * std::abs(boost_phase - t)));
        });
...
public:
    ExampleEditor() {
        ...
        // fi: eventListener callbacks only for code demo (no function)
        onMouseDrag() = [&](const visage::MouseEvent& e) {
            if (e.event_frame != &animated_line_) return;
            float x = e.windowPosition().x;
            float y = e.windowPosition().y;
            animated_line_.setBounds(x, y, width(), height());
        };

        animated_line_.onMouseEnter() = [&](const visage::MouseEvent& e) {
            setCursorStyle(visage::MouseCursor::MultiDirectionalResize);
        };

        animated_line_.onMouseExit() = [&](const visage::MouseEvent& e) {
    ...
    void resized() override {
        ...
        // member.setBounds(center, 0, width() - center, height());
    }
...
int main() {
    visage::ApplicationWindow app;  // 1.
